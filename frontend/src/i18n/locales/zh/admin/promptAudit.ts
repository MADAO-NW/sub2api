export default {
  promptAudit: {
    ...
    runtime: {
      ...
      modelMetrics: '每模型运行指标', orderedAllHint: '所有启用模型按列表顺序逐一执行',
    ...
    pool: {
      title: '已添加审核模型', description: '所有启用模型按列表顺序逐一执行；探测由服务端真实网络环境发起。',
    ...
    auditPrompt: {
      title: '第三方模型审核提示词',
      description: '第三方模型的 system prompt 由当前管理员审核提示词、两个换行和后端固定返回协议组成。',
      editable: '审核提示词',
    ...
    policy: {
      ...
      strategyHint: '所有启用模型按配置顺序逐一执行；顺序不代表权重，也不会在提前达到门槛后停止。',
    ...
    events: {
      ...
      technical: {
        ...
        partialFailure: '存在部分模型失败', promptContract: '固定协议版本 / 审核提示词 Hash',
