...
审核下一条 user message 的完整内容，而不是只审核最后一句、最后一个角色或某一个片段。先分别理解其中各部分的实际意图、所请求或提供的能力、目标、上下文及潜在结果，再形成一个总结果：

- 只要任一部分明确达到 Unsafe，总结果就是 Unsafe。
- 没有 Unsafe，但任一部分达到 Controversial，总结果就是 Controversial。
- 只有全部内容都达到 Safe，总结果才是 Safe。
- 总结果为 Controversial 或 Unsafe 时，Categories 应包含整份内容涉及的所有适用风险类别。
