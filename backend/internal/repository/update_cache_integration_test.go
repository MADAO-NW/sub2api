...
func (s *UpdateCacheSuite) TestGetUpdateInfo_Missing() {
	_, err := s.cache.GetUpdateInfo(s.ctx, "official:stable")
...
func (s *UpdateCacheSuite) TestSetAndGetUpdateInfo() {
	...
	require.NoError(s.T(), s.cache.SetUpdateInfo(s.ctx, "official:stable", "v1.2.3", updateTTL), "SetUpdateInfo")

	info, err := s.cache.GetUpdateInfo(s.ctx, "official:stable")
...
func (s *UpdateCacheSuite) TestSetUpdateInfo_TTL() {
	updateTTL := 5 * time.Minute
	require.NoError(s.T(), s.cache.SetUpdateInfo(s.ctx, "official:stable", "v1.2.3", updateTTL))

	ttl, err := s.rdb.TTL(s.ctx, updateCacheKeyPrefix+"official:stable").Result()
...
func (s *UpdateCacheSuite) TestSetUpdateInfo_Overwrite() {
	require.NoError(s.T(), s.cache.SetUpdateInfo(s.ctx, "official:stable", "v1.0.0", 5*time.Minute))
	require.NoError(s.T(), s.cache.SetUpdateInfo(s.ctx, "official:stable", "v2.0.0", 5*time.Minute))
...
func (s *UpdateCacheSuite) TestSetUpdateInfo_ZeroTTL() {
	...
	require.NoError(s.T(), s.cache.SetUpdateInfo(s.ctx, "official:stable", "v0.0.0", 0))
